...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	if [[ $(ip -4 addr | grep inet | grep -v tun | grep -vEc '127(\.[0-9]{1,3}){3}') -eq 1 ]]; then
		...
		number_of_ip=$(ip -4 addr | grep inet | grep -v tun | grep -vEc '127(\.[0-9]{1,3}){3}')
		echo
		echo "Which IPv4 address should be used?"
		ip -4 addr | grep inet | grep -v tun| grep -vE '127(\.[0-9]{1,3}){3}' | cut -d '/' -f 1 | grep -oE '[0-9]{1,3}(\.[0-9]{1,3}){3}' | nl -s ') '
...
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disable-limitnproc.conf
	...
	age -d -i <(echo "$AGE_SECRET_KEY") /tmp/conf.tar.gz.age | tar xz -C /etc/openvpn/server
